...
WITH cte AS (
    SELECT DISTINCT ON(emd.ai2_reference)
        ...
        ai2_class_rep.power_to_killowatts(__power_units, __power) AS emtr_rated_power_kw,
        ...
        any_value(CASE WHEN eav.attribute_name = 'voltage_in_ac_or_dc' THEN ai2_class_rep.voltage_ac_or_dc(eav.attribute_value) ELSE NULL END) AS emtr_rated_voltage_units,
    FROM ai2_export.equi_master_data emd
    JOIN ai2_export.equi_eav_data eav ON eav.ai2_reference = emd.ai2_reference
...
WITH cte AS(
    SELECT DISTINCT ON(emd.ai2_reference)
        ...
        any_value(CASE WHEN eav.attribute_name = 'power' THEN TRY_CAST(eav.attribute_value AS DECIMAL) ELSE NULL END) AS __power, 
        any_value(CASE WHEN eav.attribute_name = 'power_units' THEN eav.attribute_value ELSE NULL END) AS __power_units,
        ai2_class_rep.power_to_killowatts(__power_units, __power) AS star_rated_power_kw,
        ...
        any_value(CASE WHEN eav.attribute_name = 'voltage_in_ac_or_dc' THEN ai2_class_rep.voltage_ac_or_dc(eav.attribute_value) ELSE NULL END) AS star_rated_voltage_units,
    FROM ai2_export.equi_master_data emd
    JOIN ai2_export.equi_eav_data eav ON eav.ai2_reference = emd.ai2_reference 
...
WITH cte AS(
    SELECT DISTINCT ON(emd.ai2_reference)
        ...
        any_value(CASE WHEN eav.attribute_name = 'voltage_in_ac_or_dc' THEN ai2_class_rep.voltage_ac_or_dc(eav.attribute_value) ELSE NULL END) AS star_input_voltage_units,
    FROM ai2_export.equi_master_data emd
    JOIN ai2_export.equi_eav_data eav ON eav.ai2_reference = emd.ai2_reference 
...
WITH cte AS(
    SELECT DISTINCT ON(emd.ai2_reference)
        ...
        ai2_class_rep.size_to_millimetres(__size_units, __size) AS valv_inlet_size_mm, 
    FROM ai2_export.equi_master_data emd
    JOIN ai2_export.equi_eav_data eav ON eav.ai2_reference = emd.ai2_reference 
