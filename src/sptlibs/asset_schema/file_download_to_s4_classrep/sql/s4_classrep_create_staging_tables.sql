...
CREATE SCHEMA IF NOT EXISTS s4_classrep_staging;

CREATE OR REPLACE TABLE s4_classrep_staging.floc_ai2_sai_references (
    funcloc_id VARCHAR,
...
CREATE OR REPLACE TABLE s4_classrep_staging.floc_s4_aib_reference (
    funcloc_id VARCHAR,
...
CREATE OR REPLACE TABLE s4_classrep_staging.equi_ai2_sai_reference (
...
CREATE OR REPLACE TABLE s4_classrep_staging.equi_ai2_pli_reference (
    equipment_id VARCHAR,
    value_index INTEGER,
...
CREATE OR REPLACE VIEW s4_classrep_staging.vw_equi_ai2_extra_references AS 
SELECT
...
ANTI JOIN s4_classrep_staging.equi_ai2_sai_reference sai ON eav.atwrt = sai.ai2_sai_reference 
ANTI JOIN s4_classrep_staging.equi_ai2_pli_reference pli ON eav.atwrt = pli.ai2_pli_reference 
WHERE eav.charid = 'AI2_AIB_REFERENCE'
GROUP BY equipment_id;

CREATE OR REPLACE VIEW s4_classrep_staging.vw_equi_s4_aib_references AS
