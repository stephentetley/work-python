...
def gen_equi_class_tables(*, con: duckdb.DuckDBPyConnection) -> pl.DataFrame: 
    df = _get_equi_classes(con=con)
...
def _get_equi_classes(*, con: duckdb.DuckDBPyConnection) -> pl.DataFrame: 
    select_stmt = """
        SELECT DISTINCT ec.class AS class_name 
        FROM s4_fd_raw_data.classequi_classequi1 ec 
        WHERE ec.class NOT IN ('AIB_REFERENCE', 'SOLUTION_ID')
        ORDER BY class_name;
    """
