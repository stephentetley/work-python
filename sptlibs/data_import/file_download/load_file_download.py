        ...
        for line in infile.readlines():
            if payload == False:
                ...
                if len(row) > 0: 
                    _tidy_row(row)
                    rows.append(row)
    table_name1 = polars_import_utils.normalize_name(f'{entity_type}_{variant}')
    qualified_table_name = f's4_fd_raw_data.{table_name1}'
    df = pl.DataFrame(data=rows, schema=columns)
    df_renamed = polars_import_utils.normalize_df_column_names(df)
    vw_df = f'vw_df_{table_name1}'
    con.register(view_name=vw_df, python_object=df_renamed)
    sql_stmt = f'CREATE OR REPLACE TABLE {qualified_table_name} AS SELECT * FROM {vw_df};'
